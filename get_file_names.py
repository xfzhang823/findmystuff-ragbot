...
def get_file_names(
    ...
    for root, dirs, files in os.walk(directory_path):
        matched_files.extend(
            [
                os.path.join(root, f) if full_path else os.path.relpath(os.path.join(root, f), directory_path)
                for f in files
...
def main():
    dir_path = r"C:\Users\xzhan\My Drive\Job Search"
    file_list = get_file_names(dir_path, full_path=True, recursive=True, file_types=".docx", file_type_inclusive=True)
    print(file_list)
